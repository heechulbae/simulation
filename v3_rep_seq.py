...
NUM_REPAIRMEN = 1	 # Number of repairmen in the factory
OBS_TIME = 0.5		 # Observe time after which you want to observe the machines
NOW = 0

WEEKS = 4              # Simulation time in weeks
SIM_TIME = 35 # Simulation time in minutes
...
def wafer(env, wafer_name, machines):
	...
	t_arrival = env.now
	ls = []
	print("Wafer W%s enters the factory at %s and has to go through %s machines to complete it's process" %
	      (wafer_name, t_arrival, num))
	print("-------------------------------------------------------------------------------------")
	while(num != 0):
		with machines.request() as request:
			yield request
			print('Wafer W%s is being processed at Machine M%s on %s min' %
			      (wafer_name, num, env.now))
			print("------------------------------------------------------------------------------------------------------------")
			...
			while temp:
				try:
					...
					temp = 0  # Set to 0 to exit while loop.

				except simpy.Interrupt:
					temp -= env.now - start  # How much time left?
					print('M%s broke down and is in need for assistance' % num)
					print("--------------------------------------------")
					# Request a repairman. This will preempt its "other_job".
					with repairman.request(priority=1) as req:
					    print('M%s requesting at %s' % (num, env.now))
					    print("----------------------")
					    yield req
					    print('M%s got resource at %s'%(num, env.now))
					    print("----------------------")
					    yield env.timeout(REPAIR_TIME)

			print('Wafer W%s finished processing at Machine M%s in %s min' % (wafer_name, num, env.now))
			print("------------------------------------------------------------------------------------------------------------")
			num = num - 1
	t_depart = env.now
	print("Wafer W%s leaves the factory at %s" % (wafer_name, t_depart))
	wait_t.append(t_depart - t_arrival)
	print("Wait time for W%s is %s minutes" % (wafer_name, (t_depart - t_arrival)))
	print("------------------------------------------------------------------------------------------------------------")
...
print("Statistics:")
print("---------------------------------")
print("The simulation ran for %s minutes" % (SIM_TIME))
